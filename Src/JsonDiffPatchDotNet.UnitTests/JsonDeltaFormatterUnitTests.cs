...
namespace JsonDiffPatchDotNet.UnitTests
{
    ...
    public class JsonDeltaFormatterUnitTests
    {
        ...
        public string FormatJsonDeltaOperation(Operation op)
        {
            ...
            switch (op.Op)
            {
                ...
                // TODO: implement other cases when needed
                default:
                    return null;
            }
        }
        ...
        public void Format_SupportsArrayMove_Success()
        {
            ...
            Assert.AreEqual(1, operations.Count);
            AssertOperation(operations[0], OperationTypes.Move, "/items/2", null, "/items/1");
        ...
        private void AssertOperation(Operation operation, string expectedOp, string expectedPath,
            JValue expectedValue = null, string expectedFrom = null)
        {
            Assert.That(operation.Op, Is.EqualTo(expectedOp));
            Assert.That(operation.Path, Is.EqualTo(expectedPath));

            var expectedFromConstraint = expectedFrom != null ? Is.EqualTo(expectedFrom) : (IResolveConstraint) Is.Null;
            Assert.That(operation.From, expectedFromConstraint);

            if (expectedValue != null)
            {
                var value = operation.Value as JValue;
                Assert.That(value, Is.Not.Null, "Operation value was expected to be a JValue");
                Assert.That(JToken.DeepEquals(value, expectedValue));
            }
            else
            {
